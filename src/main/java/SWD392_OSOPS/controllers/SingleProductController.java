...
UserService userService;
    ...
    public String addShoesQuantityToCart(@PathVariable("id") String id, Model model, @RequestParam("quantity") int quantity) throws FileNotFoundException {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if(id.isEmpty() || id==null)   throw new FileNotFoundException("Not Found");

        model.addAttribute("isLogin", true);
        model.addAttribute("username", authentication.getName());
        cartItemService.addShoesSingleToCart(authentication.getName(),Integer.parseInt(id), quantity);
